...
let applyFilter (expression: FilterExpression) (entry: LogEntry) =
    let entryMap = unwrapLogEntry entry
...
let parseFilterLiteralExpression =
    choice
        [ parseBoolean |>> BooleanLiteral
          parseNonInfiniteDouble |>> LogLiteral.NumberLiteral
          parseSingleQuotedString |>> StringLiteral
          parseDateTimeOffset |>> TimestampLiteral ]
    |>> LiteralExpression
...
let tryCreateFilterExpression expression =
    match run (spaces >>. parseFilterExpression .>> spaces .>> eof) expression with
