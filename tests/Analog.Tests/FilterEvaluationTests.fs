...
let createLogEntry (fields: (string * Literal) list) = fields |> Map.ofList |> createEntry
...
let ``LiteralFilterExpression always evaluates to true`` () =
    ...
    let result = evalExpression expression entry
...
let ``Filter expression parsing and evaluation integration`` () =
    ...
    let result = evalExpression simpleExpression entry
    Assert.True(result)
